...
**f) swipe**(Optional) : This enable a swipe (down and up) gesture so that you will be able to change values smoothly. Default value is `false`

**g) swipeDelay**(Optional) : This value indicates an animation speed of incremental/decremental data-timepicker values. You have to set a swipe option to `true` if you would like to use the swipeDelay option. Default value is `50` ms
